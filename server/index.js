...
\\begin{center}
    ...
    \\hspace{1pt} \\faGithub \\hspace{2pt} \\texttt{GITHUB}
    \\\\ \\vspace{-3pt}
\\end{center}
...
2. Fill in the content from the resume. Escape these characters: & becomes \\& , % becomes \\% , # becomes \\# , _ becomes \\_
3. Do NOT use \\faPhone* — use \\faPhone\\ instead (with a backslash-space after).
4. Summary must be 1 line. Each job: max 3 bullets. Keep only 1 project.
5. Output ONLY raw LaTeX code. No markdown fences, no explanations, no text before \\documentclass or after \\end{document}.
6. If LinkedIn or GitHub URLs are unavailable, remove those icon lines from the header entirely.
...
${latexTemplate}`;
    ...
    // Robust cleanup: strip ALL markdown code fences (```latex, ```, etc.)
    latexCode = latexCode.replace(/```[\w]*\n?/g, '').trim();
    ...
    latexCode = latexCode.replace(/\\faPhone\*/g, '\\faPhone\\');
    ...
    if (!pdfFile) {
      return res.status(200).json({
        status: 'error',
        message: `PDF compilation failed. Open the project in Overleaf to view the logs: ${projectUrl}`,
        projectUrl,
      });
    }
